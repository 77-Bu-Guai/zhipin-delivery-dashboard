...
const tmp = path.join(process.env.TEMP || '/tmp', 'ff-snappy.sqlite');
fs.copyFileSync(src, tmp);

const db = new Database(tmp, { readonly: true });
const rows = db.prepare("SELECT key, data FROM object_data").all();

// 先分析小的 Today 和 Statistics
for (const row of rows) {
  ...
  if (!decodedKey.includes('Today') && !decodedKey.includes('Statistics') && !decodedKey.includes('netConf')) continue;
  ...
  if (decodedKey.includes('netConf')) break;
  if (decodedKey.includes('Today')) break;
}
